...
def main():
    ...
    while time_s != 0:
        spe=input("Enter a speed (0 to 900 dps):")
        position=input("Enter a distance to drive (inches): ")
        left_motor.run_to_rel_pos(speed_sp=int(spe),position_sp=int(position),
                                  stop_action=ev3.Motor.STOP_ACTION_BRAKE)
        right_motor.run_to_rel_pos(speed_sp=int(spe),position_sp=int(
            position)*90,
                                   stop_action=ev3.Motor.STOP_ACTION_BRAKE)
